...
def registrar_usuario(email, nombre, codigo_estudiante, password):
    """Registrar un nuevo usuario en la base de datos SQLite"""
    ...
    try:
        ...
        cur.execute("SELECT codigo_estudiante FROM users WHERE codigo_estudiante = ?", (codigo_estudiante,))
        if cur.fetchone():
            cur.close()
            conn.close()
        ...
        cur.execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (email, nombre.strip(), codigo_estudiante, password_hash)
        )
        conn.commit()
        cur.close()
        conn.close()
...
def autenticar_usuario(email, password):
    """Autenticar usuario con email y contraseña usando SQLite"""
    conn = get_db_connection()
    if conn is None:
        return None
    try:
