...
def register():
    try:
        ...
        try:
            cursor.execute(
                'INSERT INTO users (email, name, student_code, password_hash) VALUES (?, ?, ?, ?)',
...
def get_datasets():
    try:
        ...
        cursor.execute(
            'SELECT id, name, created_at FROM datasets WHERE user_id = ? ORDER BY created_at DESC',
            (session['user_id'],)
        )
        ...
        return jsonify({'datasets': datasets_list}), 200
    except Exception as e:
...
def run_regression():
    try:
        ...
        cursor.execute('SELECT data FROM datasets WHERE id = ? AND user_id = ?', (dataset_id, session['user_id']))
